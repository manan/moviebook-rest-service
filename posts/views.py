...
class NewsFeed(generics.ListAPIView):
    ...
    def get_queryset(self):
        ...
        for person in people_following:
            for post in person.posts.filter(upload_date__gte=datetime.now() - timedelta(days=30)):
                newsfeed.append(post)
        for post in profile.posts.filter(upload_date__gte=datetime.now() - timedelta(days=30)):
            newsfeed.append(post)
